...
.post(({query}, res, next) ->
	if query.unset is 'yes'
		e.likes.remove query.user, query.product, (err) =>
			if err?
				return next err
		...
		e.dislikes.remove query.user, query.product, (err) =>
			if err?
				return next err

			e.likes.add query.user, query.product, (err) =>
				if err?
					return next err
...
.post(({query}, res, next) ->
	if query.unset is 'yes'
		...
		e.likes.remove query.user, query.product, (err) =>
			...
			e.dislikes.add query.user, query.product, (err) =>
				if err?
					return next err
		...
		suggestions: (done) =>
			e.suggestions.forUser query.user, (err, suggestions) =>
				if err?
					...
					_.findWhere products, id: suggestion.item
